...
const BookingPage = () => {
  ...
  return (
    <div>
      ...
      {bookings.length === 0 ? (
        <div className="text-center py-20 text-muted-foreground text-sm">No bookings yet.</div>
      ) : (
                ...
                <tr key={booking._id} className="hover:bg-muted transition">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <img src={booking.propertyImage} alt={booking.propertyTitle} className="w-12 h-10 rounded-lg object-cover flex-shrink-0" />
                      <span className="font-medium text-foreground line-clamp-1">{booking.propertyTitle}</span>
                    </div>
                  </td>
                  <td className="px-5 py-4 text-muted-foreground">{booking.location}</td>
                  ...
                  <td className="px-5 py-4">
                    <span className={`text-xs font-semibold px-3 py-1 rounded-full capitalize
                      ${booking.paymentStatus === "paid" ? "bg-green-100 text-green-700" : "bg-gray-100 text-muted-foreground"}`}>
